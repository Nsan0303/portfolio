...
def create_file(path: str, content: str = ""):
    ...
    os.makedirs(os.path.dirname(path + extension), exist_ok=True)
...
def main():
    ...
    if file_extension == "html":
        # テンプレート読み込み
        with open(os.path.join("blogsys", "test", "template", "template.html"), "r", encoding="utf-8") as f:
            template_html = f.read()
        with open(os.path.join("blogsys", "test", "template", "template.css"), "r", encoding="utf-8") as f:
            template_css = f.read()
        with open(os.path.join("blogsys", "test", "template", "template.js"), "r", encoding="utf-8") as f:
            template_js = f.read()
        ...
        create_file(html_path, template_html)
        create_file(css_path, template_css)
        create_file(js_path, template_js)
